...
fn main() {
	...
	app.static_compressed_max_size = 524288 // Maximum file size for auto-compression is 512 KB (default: 1MB)
	...
	app.use(veb.encode_content[Context]())
...
- **Priority order**: When both `.zst` and `.gz` files exist for the same source file,
  veb will serve `.zst` if the client supports zstd, otherwise `.gz` if gzip is supported.
...
- Large files (> threshold) are always streamed, ensuring low memory usage even for
  large assets.
- The `encode_content` middleware automatically chooses zstd or gzip based on client support.
  You can also use `encode_zstd` or `encode_gzip` for specific compression.
- If caching fails (e.g., on read-only filesystems), veb automatically falls
  back to serving compressed content from memory. You can set `static_compressed_max_size = 0`
