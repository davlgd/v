...
fn testsuite_begin() {
	...
	app.Middleware.route_use('/content', veb.encode_content[Context]())
...
// Verifies that encode_content uses zstd when client supports it
fn test_encode_content_zstd() {
...
// Verifies that encode_content falls back to gzip when client doesn't support zstd
fn test_encode_content_gzip_fallback() {
	mut req := http.new_request(.get, '${localserver}/content', '')
...
// Verifies that encode_content sends uncompressed when no encoding is supported
fn test_encode_content_no_compression() {
	mut req := http.new_request(.get, '${localserver}/content', '')
