...
mut:
	...
	// Configuration for static file compression (zstd/gzip, set by serve_if_static)
...
fn (mut ctx Context) send_file(content_type string, file_path string) Result {
	...
	max_size_bytes := ctx.static_compressed_max_size

	// Try to serve pre-compressed files if static compression is enabled
	if ctx.enable_static_compression && (client_accepts_zstd || client_accepts_gzip) {
		orig_mtime := os.file_last_mod_unix(file_path)

		// Try zstd first (better compression), then gzip
