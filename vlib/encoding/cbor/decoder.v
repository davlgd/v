...
// negatives, floats, or other major types.
...
// unpack_bool reads a CBOR boolean (simple 20/21).
pub fn (mut u Unpacker) unpack_bool() !bool {
	...
	return type_mismatch(u.pos - 1, 'bool', b)
}
...
pub fn (mut u Unpacker) unpack_null() ! {
	b := u.read_byte()!
	if b != 0xf6 {
		return type_mismatch(u.pos - 1, 'null', b)
	}
}
...
pub fn (mut u Unpacker) unpack_text() !string {
	...
	return u.read_text_chunk(int(size))!
}

@[direct_array_access]
fn (mut u Unpacker) read_text_chunk(size int) !string {
	if u.pos + size > u.data.len {
		return eof_needing(u.pos, size, u.data.len - u.pos)
	}
	bytes_start := u.pos
	u.pos += size
	if u.opts.validate_utf8 && !utf8_validate_slice(u.data, bytes_start, size) {
...
pub fn (mut u Unpacker) unpack_bytes() ![]u8 {
	...
	size := u.read_arg(info)!
	return u.read_bytes_chunk(int(size))!
}

@[direct_array_access]
fn (mut u Unpacker) read_bytes_chunk(size int) ![]u8 {
	if u.pos + size > u.data.len {
		return eof_needing(u.pos, size, u.data.len - u.pos)
	}
	out := u.data[u.pos..u.pos + size].clone()
	u.pos += size
	return out
}
...
fn (mut u Unpacker) read_indef_bytes() ![]u8 {
	...
	for {
		...
		size := u.read_arg(info)!
		acc << u.read_bytes_chunk(int(size))!
	}
	return acc
}
...
// must follow up by reading the tag content.
	...
	match major {
		...
		6 {
			...
			match info {
				...
				24 {
					u.pos++
				} // simple value 24..255 (one extra byte)
				25 {
					u.pos += 2
				} // half
				26 {
